...
function rehydrateStyles(
  ...
  unreadPropStyles: TransitionStyle[] | null,
...
const TransitionMotion: React.FC<TransitionProps> = ({
  defaultStyles,
  styles,
  children,
  ...
  const unreadPropStylesRef = useRef<TransitionStyle[] | null>(null)

  const defaultState = (): TransitionMotionState => {
  ...
  const [state, setState] = useState<TransitionMotionState>(() => defaultState())

  const clearUnreadPropStyle = (unreadPropStyles: TransitionStyle[]): void => {
    const [
      mergedPropsStyles,
      currentStyles,
      currentVelocities,
      ...
      willLeave,
      didLeave,
      state.mergedPropsStyles,
      unreadPropStyles,
      state.currentStyles,
      state.currentVelocities,
      state.lastIdealStyles,
      state.lastIdealVelocities,
    )
  ...
  const startAnimationIfNecessary = (): void => {
    ...
    animationIDRef.current = defaultRaf((timestamp) => {
      ...
      // > if execution passes the conditional if (this.unmountingRef.current), then
      ...
      if (
        ...
        // no need to cancel animationIDRef.current here; shouldn't have any in flight
      ...
      const currentFrameCompletion = (accumulatedTimeRef.current - Math.floor(accumulatedTimeRef.current / msPerFrame) * msPerFrame) / msPerFrame
  ...
  useEffect(() => {
    if (unreadPropStylesRef.current)
      clearUnreadPropStyle(unreadPropStylesRef.current)

    if (typeof styles === 'function') {
      unreadPropStylesRef.current = styles(
        rehydrateStyles(
          state.mergedPropsStyles,
          unreadPropStylesRef.current,
          state.lastIdealStyles,
  ...
  const renderedChildren = children(hydratedStyles)
